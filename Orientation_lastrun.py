...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Noman\\Desktop\\Anushka\\Orientation_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "Welcome_Scree" ---
    textWelcomeMessage = visual.TextStim(win=win, name='textWelcomeMessage',
        ...
        languageStyle='LTR',
        depth=0.0);
    textWelcomeContinuos = visual.TextStim(win=win, name='textWelcomeContinuos',
        text='The experiment is about to start is that okay',
        font='Arial',
        pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Prepare to start Routine "Welcome_Scree" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Welcome_Scree.started', globalClock.getTime(format='float'))
    # Run 'Begin Routine' code from code_2
    triggerSent = False
    # keep track of which components have finished
    Welcome_ScreeComponents = [textWelcomeMessage, textWelcomeContinuos]
    for thisComponent in Welcome_ScreeComponents:
    ...
    # --- Run Routine "Welcome_Scree" ---
    routineForceEnded = not continueRoutine
    while continueRoutine and routineTimer.getTime() < 52.0:
        # get current time
        ...
        if textWelcomeMessage.status == STARTED:
            ...
            if tThisFlipGlobal > textWelcomeMessage.tStartRefresh + 2-frameTolerance:
        ...
        # *textWelcomeContinuos* updates
        
        # if textWelcomeContinuos is starting this frame...
        if textWelcomeContinuos.status == NOT_STARTED and tThisFlip >= 2-frameTolerance:
            # keep track of start time/frame for later
            textWelcomeContinuos.frameNStart = frameN  # exact frame index
            textWelcomeContinuos.tStart = t  # local t and not account for scr refresh
            textWelcomeContinuos.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(textWelcomeContinuos, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'textWelcomeContinuos.started')
            # update status
            textWelcomeContinuos.status = STARTED
            textWelcomeContinuos.setAutoDraw(True)
        
        # if textWelcomeContinuos is active this frame...
        if textWelcomeContinuos.status == STARTED:
            # update params
            pass
        
        # if textWelcomeContinuos is stopping this frame...
        if textWelcomeContinuos.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > textWelcomeContinuos.tStartRefresh + 50-frameTolerance:
                # keep track of stop time/frame for later
                textWelcomeContinuos.tStop = t  # not accounting for scr refresh
                textWelcomeContinuos.tStopRefresh = tThisFlipGlobal  # on global time
                textWelcomeContinuos.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'textWelcomeContinuos.stopped')
                # update status
                textWelcomeContinuos.status = FINISHED
                textWelcomeContinuos.setAutoDraw(False)
    ...
    # --- Ending Routine "Welcome_Scree" ---
    for thisComponent in Welcome_ScreeComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Welcome_Scree.stopped', globalClock.getTime(format='float'))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
        routineTimer.addTime(-52.000000)
    thisExp.nextEntry()
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('Good_Bye.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    Good_ByeComponents = []
    ...
    for thisComponent in Good_ByeComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('Good_Bye.stopped', globalClock.getTime(format='float'))
    thisExp.nextEntry()
    # the Routine "Good_Bye" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
