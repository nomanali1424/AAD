...
expName = 'Trail2'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\Noman\\Desktop\\Anushka\\Trail2_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text = visual.TextStim(win=win, name='text',
        text='The experiment is about to start hold tight',
        font='Arial',
    ...
    image = visual.ImageStim(
        win=win,
        name='image', 
        image='C:/Users/Noman/Downloads/Untitled.png', mask=None, anchor='center',
    ...
    trials_loop = data.TrialHandler(nReps=10.0, method='random', 
    ...
    for thisTrials_loop in trials_loop:
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if key_resp.status == STARTED and not triggerSent: #If the stimulus component has started and the trigger has not yet been sent. Change 'stimulus' to match the name of the component you want the trigger to be sent at the same time as
        ...
        continueRoutine = True
        # update component parameters for each repeat
        thisExp.addData('Picture.started', globalClock.getTime(format='float'))
        # keep track of which components have finished
        PictureComponents = [image]
        ...
        for thisComponent in PictureComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Picture.stopped', globalClock.getTime(format='float'))
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    # completed 10.0 repeats of 'trials_loop'
